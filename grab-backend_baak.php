...
function omj_grab_page()
{
  ...
  if($issubmit) {
    ...
    $datachk = rs_gws_local($base,base64_encode($u),'grab_chk');
    #$datachk = 'yH4';
    if($datachk=='z0nk'){
      ...
      #echo base64_encode(json_encode($datalive));
      //print_r($datalive);
      #exit;
      ...
      if($saveit=='yH4') {
        ...
        $dtupdate = array('turl'=>$u, 'gstatus'=>'grabbed');
        $apdet = rs_gws_local($base,base64_encode(json_encode($dtupdate)),'grab_update');
        if($apdet=='yH4') {
          // transisi di sini
          // cek transisi.
          if($transit){
...
function omj_graball_page(){
  ...
  <div class="inside">';
  /*01. Statistik */
  $stat = rs_gws_local($base, 'senddata', 'graball_stats');
  $stat = json_decode($stat);
  $alltarget = $stat->alltarget;
  $allgrabbed = $stat->allgrabbed;
  $allskipped = $stat->allskipped;
  $turl = $stat->turl;
